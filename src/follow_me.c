...
#define FOLLOWER_GFX_NONE          0xFFFF
...
static bool8 SetUpFollowerFromParty(u16 followerFlags);
static struct ObjectEventTemplate BuildPartyFollowerTemplate(void);
...
#define IN_RANGE(state, min, max) ((state) >= (min) && (state) <= (max))
...
static u8 DetermineFollowerState(struct ObjectEvent* follower, u8 state, u8 direction)
{
    ...
    if (IN_RANGE(state, MOVEMENT_ACTION_WALK_SLOW_DOWN, MOVEMENT_ACTION_WALK_SLOW_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_WALK_SLOW_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_WALK_NORMAL_DOWN, MOVEMENT_ACTION_WALK_NORMAL_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_WALK_NORMAL_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_JUMP_2_DOWN, MOVEMENT_ACTION_JUMP_2_RIGHT))
    {
        ...
        || ((newState - direction) >= 0x84 && (newState - direction) <= 0x87)) // Previously jumped
        {
            ...
            RETURN_STATE(MOVEMENT_ACTION_JUMP_2_DOWN, direction);
        }

        gSaveBlock2Ptr->follower.delayedState = MOVEMENT_ACTION_JUMP_2_DOWN;
        RETURN_STATE(MOVEMENT_ACTION_WALK_NORMAL_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_WALK_FAST_DOWN, MOVEMENT_ACTION_WALK_FAST_RIGHT))
    {
    ...
    else if (IN_RANGE(state, MOVEMENT_ACTION_WALK_FASTER_DOWN, MOVEMENT_ACTION_WALK_FASTER_RIGHT))
    {
        if (MetatileBehavior_IsIce(follower->currentMetatileBehavior) || MetatileBehavior_IsTrickHouseSlipperyFloor(follower->currentMetatileBehavior))
            follower->disableAnim = TRUE;
    ...
    else if (IN_RANGE(state, MOVEMENT_ACTION_RIDE_WATER_CURRENT_DOWN, MOVEMENT_ACTION_RIDE_WATER_CURRENT_RIGHT))
    {
    ...
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_FACE_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_FACE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_FACE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_POP_WHEELIE_DOWN, MOVEMENT_ACTION_ACRO_POP_WHEELIE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_POP_WHEELIE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_END_WHEELIE_FACE_DOWN, MOVEMENT_ACTION_ACRO_END_WHEELIE_FACE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_END_WHEELIE_FACE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_HOP_FACE_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_HOP_FACE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_HOP_FACE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_HOP_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_HOP_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_HOP_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_JUMP_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_JUMP_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_JUMP_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_IN_PLACE_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_IN_PLACE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_IN_PLACE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_POP_WHEELIE_MOVE_DOWN, MOVEMENT_ACTION_ACRO_POP_WHEELIE_MOVE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_POP_WHEELIE_MOVE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_ACRO_WHEELIE_MOVE_DOWN, MOVEMENT_ACTION_ACRO_WHEELIE_MOVE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_ACRO_WHEELIE_MOVE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_SLIDE_DOWN, MOVEMENT_ACTION_SLIDE_RIGHT))
    {
        RETURN_STATE(MOVEMENT_ACTION_SLIDE_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_PLAYER_RUN_DOWN, MOVEMENT_ACTION_PLAYER_RUN_RIGHT))
    {
    ...
    else if (IN_RANGE(state, MOVEMENT_ACTION_JUMP_SPECIAL_DOWN, MOVEMENT_ACTION_JUMP_SPECIAL_RIGHT))
    {
        gSaveBlock2Ptr->follower.delayedState = MOVEMENT_ACTION_JUMP_SPECIAL_DOWN;
        RETURN_STATE(MOVEMENT_ACTION_WALK_NORMAL_DOWN, direction);
    }
    else if (IN_RANGE(state, MOVEMENT_ACTION_JUMP_DOWN, MOVEMENT_ACTION_JUMP_RIGHT))
    {
        gSaveBlock2Ptr->follower.delayedState = MOVEMENT_ACTION_JUMP_DOWN;
        RETURN_STATE(MOVEMENT_ACTION_WALK_NORMAL_DOWN, direction);
    }
    #ifdef MOVEMENT_ACTION_RUN_DOWN_SLOW
    else if (IN_RANGE(state, MOVEMENT_ACTION_RUN_DOWN_SLOW, MOVEMENT_ACTION_RUN_RIGHT_SLOW))
    {
        if (gSaveBlock2Ptr->follower.flags & FOLLOWER_FLAG_HAS_RUNNING_FRAMES)
...
void SetFollowerSprite(u8 spriteIndex)
{
    ...
    if (gSaveBlock2Ptr->follower.map.id == DEFAULT_FOLLOWER_LOCAL_ID)
        clone = BuildPartyFollowerTemplate();
    else
...
static struct ObjectEventTemplate BuildPartyFollowerTemplate(void)
{
    ...
    template.graphicsId = gSaveBlock2Ptr->follower.graphicsId;
    template.kind = OBJ_KIND_NORMAL;
    template.objUnion.normal.elevation = 0;
    ...
    template.objUnion.normal.trainerType = 0;
    template.objUnion.normal.trainerRange_berryTreeId = 0;
    template.script = gSaveBlock2Ptr->follower.script;
    template.flagId = 0;
...
void CreateFollowerAvatar(void)
{
    ...
    player = &gObjectEvents[gPlayerAvatar.objectEventId];
    template = GetObjectEventTemplateByLocalIdAndMap(gSaveBlock2Ptr->follower.map.id, gSaveBlock2Ptr->follower.map.number, gSaveBlock2Ptr->follower.map.group);
    if (gSaveBlock2Ptr->follower.map.id == DEFAULT_FOLLOWER_LOCAL_ID || template == NULL)
        clone = BuildPartyFollowerTemplate();
    else
        clone = *template;
    ...
    clone.objUnion.normal.movementType = MOVEMENT_TYPE_NONE; //Doesn't get to move on its own
...
static bool8 SetUpFollowerFromParty(u16 followerFlags)
{
    ...
    if (graphicsId == FOLLOWER_GFX_NONE)
        return FALSE;

    gSaveBlock2Ptr->follower.inProgress = TRUE;
    gSaveBlock2Ptr->follower.objId = OBJECT_EVENTS_COUNT;
    gSaveBlock2Ptr->follower.graphicsId = graphicsId;
    gSaveBlock2Ptr->follower.map.id = DEFAULT_FOLLOWER_LOCAL_ID;
    gSaveBlock2Ptr->follower.map.number = gSaveBlock1Ptr->location.mapNum;
    gSaveBlock2Ptr->follower.map.group = gSaveBlock1Ptr->location.mapGroup;
    gSaveBlock2Ptr->follower.script = EventScript_PartyFollower;
    gSaveBlock2Ptr->follower.flag = 0;
    gSaveBlock2Ptr->follower.flags = followerFlags;
    gSaveBlock2Ptr->follower.createSurfBlob = 0;
    gSaveBlock2Ptr->follower.comeOutDoorStairs = 0;
    gSaveBlock2Ptr->follower.currentSprite = FOLLOWER_SPRITE_INDEX_NORMAL;
    gSaveBlock2Ptr->follower.delayedState = 0;
    gSaveBlock2Ptr->follower.locked = 0;
...
void DestroyFollower(void)
{
    if (gSaveBlock2Ptr->follower.inProgress)
    {
        ...
        if (gSaveBlock2Ptr->follower.flag != 0)
